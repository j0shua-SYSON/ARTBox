...
int main(int argc, char **argv) {
    ...
    const char *contract = argc == 4 ? argv[3] : "reference";
    int storage = !strcmp(contract, "storage"), stack = !strcmp(contract, "stack");
    ...
    void *symbol = dlsym(library, stack ? "artbox_art_stack_check" :
    ...
    if (result != expected_cases || *(uint64_t *)first != UINT64_C(0x123456789abcdef0) ||
        *(uint64_t *)second != UINT64_C(0xfedcba9876543210) || observations[0] != (uint32_t)poison ||
        observations[1] != (poison && !stack ? 0u-first_bits : first_bits) ||
