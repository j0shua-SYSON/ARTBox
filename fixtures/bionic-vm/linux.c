...
int64_t artbox_stub_artbox_bionic_syscall(uint64_t n, uint64_t a0, uint64_t a1, uint64_t a2,
    ...
    /* Only the fixture's explicit operations may enter the real Linux ABI. */
    if (n != 222 && n != 226 && n != 215 && n != 233 && n != 172 && n != 178 &&
        n != 96 && n != 113 && n != 135 && n != 278) return -38;
    int saved = errno;
...
int main(void) {
    ...
    if (cases != 35 || system_cases != 53 || errno != EDOM) {
    ...
    printf("{\"cases\":35,\"page_size\":%ld,\"elapsed_ns\":%" PRId64 ",\"system_cases\":53,\"system_elapsed_ns\":%" PRId64 "}\n",
